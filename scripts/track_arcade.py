...
# example:
# blender -b --python track_arcade.py -- /Volumes/external/exports/racetrack_arcade_v1.fbx
# ---------------------------------
argv = sys.argv
if "--" in argv:
    out_path = argv[argv.index("--") + 1]
else:
    out_path = "/Volumes/external/exports/racetrack_arcade_v1.fbx"
...
# Tweak these to reshape the circuit.
...
def build_road(left_pts, right_pts, collection, material):
    n = len(left_pts)
    verts = [p.copy() for p in left_pts] + [p.copy() for p in right_pts]
    faces = []
...
def scatter_trees(centerline, left_normals, road_width, count, collection, trunk_material, leaves_material):
    ...
    while placed < count and attempts < count * 12:
        ...
        # keep some space around start line
        ...
        base = centerline[idx] + normal * (road_width * 0.5 + WALL_THICKNESS + TREE_CLEARANCE)
...
road_mat = make_material("Road_Mat", (0.08, 0.08, 0.08, 1.0), roughness=0.95)
...
start_line_mat = make_material("StartLine_Mat", (1.0, 1.0, 1.0, 1.0), roughness=0.5)
...
left_wall = build_wall(left_edge, left_normals, WALL_THICKNESS, WALL_HEIGHT, "LeftWall", collection, wall_mat)
right_wall = build_wall(right_edge, [-n for n in left_normals], WALL_THICKNESS, WALL_HEIGHT, "RightWall", collection, wall_mat)
