...
# example:
# blender -b --python track_arcade_big.py -- /Volumes/external/exports/racetrack_arcade_big.glb
# ---------------------------------
argv = sys.argv
if "--" in argv:
    out_path = argv[argv.index("--") + 1]
else:
    out_path = "/Volumes/external/exports/racetrack_arcade_big.glb"
...
CHECKPOINT_HEIGHT = 5.0
CHECKPOINT_THICKNESS = 0.7
CHECKPOINT_EXTRA_WIDTH = 2.0

# curbs
CURB_WIDTH = 1.2
CURB_HEIGHT = 0.18
CURB_STEP = 2         # alternate every 2nd sample for red/white pattern
CURB_Z_OFFSET = 0.06  # lift above road to prevent Z-fighting
...
TRACK_CONTROL_POINTS = [
    ...
    for p in BASE_TRACK_CONTROL_POINTS
]
...
def make_material(name, color, roughness=0.8, metallic=0.0, specular=0.5, emission=(0,0,0,1), emission_strength=0.0):
    """Create a PBR material with full Principled BSDF controls."""
...
def build_curbs(centerline, tangents, left_normals, road_width, collection, mat_a, mat_b):
    """Build curbs as continuous ribbon segments — no gaps between red and white."""
    objs = []
    n = len(centerline)
    half_road = road_width * 0.5
    z_up = Vector((0.0, 0.0, CURB_Z_OFFSET))
    z_top = Vector((0.0, 0.0, CURB_HEIGHT + CURB_Z_OFFSET))

    # Group consecutive samples into segments that alternate color
    seg_start = 0
    seg_index = 0

    while seg_start < n:
        seg_end = min(seg_start + CURB_STEP, n)
        curb_mat = mat_a if (seg_index % 2 == 0) else mat_b

        # Collect the sample indices for this segment (plus one extra to close the quad)
        indices = list(range(seg_start, seg_end + 1))
        # Wrap the last index
        indices = [idx % n for idx in indices]
        ...
        for side_name, side_sign in [("CurbL", 1.0), ("CurbR", -1.0)]:
            ...
            for idx in indices:
                ...
                # Outer edge
                outer = c + lft * ((half_road + CURB_WIDTH) * side_sign)

                # Bottom two verts, then top two
                verts.append(inner + z_up)
                verts.append(outer + z_up)
                verts.append(inner + z_top)
                verts.append(outer + z_top)
            ...
            for k in range(nv - 1):
                ...
                # Inner face
                faces.append([base + 2, base + 0, nxt + 0, nxt + 2])
                # Bottom face
                faces.append([base + 0, base + 1, nxt + 1, nxt + 0])

            obj_name = f"{side_name}_{seg_start:03d}"
            objs.append(add_mesh_object(obj_name, verts, faces, collection, curb_mat))
...
# Asphalt: dark, rough, slightly warm
road_mat = make_material("Road_Mat",
...
# Concrete walls: slightly rough, light grey with a hint of blue
wall_mat = make_material("Wall_Mat",
...
# Grass ground: natural green, very rough
ground_mat = make_material("Ground_Mat",
    color=(0.12, 0.28, 0.08, 1.0),
    roughness=1.0,
    metallic=0.0,
    specular=0.1,
)

# Start line: bright white, slightly reflective like fresh paint
start_line_mat = make_material("StartLine_Mat",
...
# Checkpoints: semi-transparent trigger volumes (visible in Blender, hidden in game)
checkpoint_mat = make_material("Checkpoint_Mat",
    color=(1.0, 0.75, 0.1, 0.3),
    roughness=0.4,
    metallic=0.0,
)

# Curbs: classic red/white racing stripes
curb_red_mat = make_material("Curb_Red_Mat",
    color=(0.75, 0.08, 0.08, 1.0),
    roughness=0.65,
    metallic=0.0,
    specular=0.4,
)
curb_white_mat = make_material("Curb_White_Mat",
...
# Center lane stripes: slightly reflective paint
stripe_mat = make_material("Stripe_Mat",
    color=(0.90, 0.88, 0.65, 1.0),
    roughness=0.40,
    metallic=0.0,
    specular=0.5,
)
